...
Copyright 2013, Ross Mulcare, rossmulcare.net
...
def get_wind(parsed):
    ...
    speed_mph = int(round(kph * .621371, 0))
    if speed < 1:
        description = 'Calm'
    elif speed < 4:
    ...
    return description + ' ' + str(speed_mph) + 'mph (' + degrees + ')'
...
def weather(bot, trigger):
    ...
    #strip out 'Yahoo! Weather'
    location = location.lstrip('Yahoo! Weather - ')
    cover = get_cover(parsed)
    temp = get_temp(parsed)
#    pressure = get_pressure(parsed)
    wind = get_wind(parsed)
    bot.say(u'%s: %s, %s, %s' % (location, cover, temp, wind))
